...
@interface NITextField(Private) <UIPopoverControllerDelegate>
...
-(UIView*)inputView
{
    ...
    else if(type == NITextFieldTypeDatePicker)
    {
...
-(void)segChanged:(id)sender
{
    ...
    {
        if([niDelegate respondsToSelector:@selector(previous:)])
            [niDelegate previous:self];
    }
    else
    {
        if([niDelegate respondsToSelector:@selector(next:)])
           [niDelegate next:self];
    }
